...
#define rgblight_timer_init() do { DDRD |= (1<<4);} while(0)
void rgblight_timer_enable(void)  { PORTD &= ~(1<<4);}
void rgblight_timer_disable(void) { PORTD |= (1<<4);}
#define rgblight_timer_enabled (~PORTD & (1<<4))
#define RGBLED_TEMP  RGBLED_NUM
#define RGB_INDICATOR_NUM 0
...
#ifdef RGBLIGHT_FADING_ONOFF_ENABLE
        // 如果使用fading，关闭时，所有灯需要清0，防止下次启用时先闪亮一下
        rgblight_clear();
        rgblight_set();
        rgb_fading_index = 0; 
#endif
